...
Ans: Select  title, description, rating,length from film where (length/60) >=3;
...
Ans: select title, description ,release_year ,(rental_duration * rental_rate) as total_rental_cost from film where length(description) between 100 and 120;
...
Ans: select title, description, special_features, length,rental_duration from film where special_features like "%behind the scenes%" and length <120 and rental_duration between 5 and 7 order by length desc limit 10;
...
Ans: select c.first_name as customer_first_name,c.last_name as customer_last_name,a.first_name as actor_first_name,a.last_name as actor_last_name from customer c  left join actor a on c.last_name =a.last_name;
...
Ans: select s.first_name,s.last_name,a.address,a.address2,c.city,a.district,a.postal_code from staff s left join address a on s.address_id=a.address_id left join city c on a.city_id=c.city_id;